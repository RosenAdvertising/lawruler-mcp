#!/usr/bin/env python3
"""Pluggable credential storage for lawruler-mcp.
...
Storage order (read):  OS keyring  ->  process env  ->  ~/.lawruler-mcp/.env file
...
user opts out of keyring with ``LAWRULER_MCP_USE_KEYRING=0``.
...
SERVICE_NAME = "lawruler-mcp"
CONFIG_DIR = Path.home() / ".lawruler-mcp"
ENV_FILE = CONFIG_DIR / ".env"
_USE_KEYRING_FLAG = "LAWRULER_MCP_USE_KEYRING"
# ---------------------------------------------------------------------------
...
def _keyring_enabled() -> bool:
    ...
    Opt out with ``LAWRULER_MCP_USE_KEYRING=0`` (or false/no/off). A failing or
